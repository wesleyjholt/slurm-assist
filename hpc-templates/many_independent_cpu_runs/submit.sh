...
HPC_DIR=many_independent_cpu_runs  # Path to the directory containing the HPC scripts
INPUT_FILE=${HPC_DIR}/input_data.pkl  # Contains all input data in one giant list.
BATCHED_DATA_DIR=${HPC_DIR}/data_batches  # Directory to store batched data 
BATCHED_RESULTS_DIR=${HPC_DIR}/results_batches  # Directory to store batched results
OUTPUT_FILE=${HPC_DIR}/results.pkl  # File to store results
...
SUBMIT_CONFIRMATION=`sbatch --array 1-$ARRAY_SIZE --mem-per-cpu $MEM_PER_CPU --ntasks $NTASKS_PER_JOB ${HPC_DIR}/_run.sh $BATCHED_DATA_DIR $BATCHED_RESULTS_DIR $OUTPUT_FILE $HPC_DIR $ENVIRONMENT`
JOB_ID=$(echo "$SUBMIT_CONFIRMATION" | grep -o -E '[0-9]+')

# Merge results
sbatch --dependency afterok:$JOB_ID --mem-per-cpu $MEM_PER_CPU_POST --ntasks 1 ${HPC_DIR}/_merge_results.sh $ARRAY_SIZE $NTASKS_PER_JOB $BATCHED_RESULTS_DIR $OUTPUT_FILE $HPC_DIR $ENVIRONMENT