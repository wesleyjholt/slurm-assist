...
DependencyIdsList = list[list[int]]
DependencyCondList = list[str]
DependencyGenFunc = Callable[[tuple[int], Any], tuple[tuple[DependencyIdsList, DependencyCondList], Any]]
...
class SerialJobs(JobGroup):
    ...
    job_group_gen_fns: Union[JobGroupGenFunc, list[JobGroupGenFunc]]
        Job group generator functions. Takes as input a config object and state object, 
        and returns a job group object (e.g., SingleJob, EmbarrassinglyParallelJobs). 
        Can be a single function or a list of such functions.
    config_gen_fns: Union[ConfigGenFunc, list[ConfigGenFunc]]
        Config object generator functions. Takes as input a config object and a state 
        object, and outputs another 
