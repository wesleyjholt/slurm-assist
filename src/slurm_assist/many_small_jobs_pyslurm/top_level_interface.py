...
class ManySmallJobs(dict):
    def __init__(
        ...
        # Add options to slurm (if needed)
        if 'environment' not in self['main_slurm_args']:
            self['main_slurm_args']['environment'] = 'ALL'
        if 'environment' not in self['merge_slurm_args']:
            self['main_slurm_args']['environment'] = 'ALL'
        ...
        main_job_script = main_script_template.render(dict(
        ...
        merge_job_script = merge_script_template.render(dict(
            container_image=self['container_image'],
